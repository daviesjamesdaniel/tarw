    ...
    fn fetch(&mut self, mailbox_hash: MailboxHash) -> ResultStream<Vec<Envelope>> {
        ...
        Ok(Box::pin(try_fn_stream(|emitter| async move {
            let id = state.connection.lock().await?.id.clone();
            {
                ...
                // (e.g. a reconnect that rebuilds the mailbox list) - indexing
                // with `[]` panicked on exactly that race, and since fetch()
                // is driven straight from a Tauri IPC command on the main
                // thread, the panic couldn't unwind and aborted the whole
                // process. Retry with backoff first, the same way `idle()`
                // in watch.rs waits out INBOX not being in the map yet -
                // this is a rebuild race, not a real "gone forever" case, so
                // it should heal itself within a few hundred ms rather than
                // surfacing an error the user has to manually retry.
                let mut retries = 0;
                loop {
        ...
        Ok(Box::pin(async move {
            ...
            *mailboxes = new_mailboxes;
            ...
            for m in mailboxes.values_mut() {
                ...
                to_persist.push((m.hash(), m.path().to_string(), m.imap_path().to_string(), m.separator, m.no_select, m.special_usage(), m.is_subscribed()));
            }
            /*
            ...
            */
            ...
            let result = mailboxes
                ...
                .collect();
            drop(mailboxes);
            for (hash, path, imap_path, separator, no_select, special_use, is_subscribed) in to_persist {
                uid_store.update_mailbox_metadata(hash, &path, &imap_path, separator, no_select, special_use, is_subscribed)?;
            }
    ...
    fn copy_messages(
        ...
        let uids: SmallVec<[UID; 64]> = {
            ...
            env_hashes
                .iter()
                .filter_map(|env_hash| {
                    let resolved = hash_index_lck.get(&env_hash).cloned();
                    resolved.map(|(uid, _)| uid)
                })
                .collect()
        };
