...
impl ImapConnection {
    ...
    pub async fn resync_condstore(
        ...
        // ResyncCache (fetch.rs) right after its own init_mailbox() call -
        // that call leaves the connection in EXAMINE state, not SELECT, so
        // force: false doesn't change anything there - but also from
