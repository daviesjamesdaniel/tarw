...
use crate::{
    backends::{
        EnvelopeHashBatch, FlagOp, MailboxHash, RefreshEvent, RefreshEventKind, SpecialUsageMailbox,
        TagHash,
    },
...
impl ImapCache for Sqlite3Cache {
    ...
    fn init_mailbox(
        ...
        let tx = self
            .connection
            .transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)?;
        // A genuine UIDVALIDITY mismatch (the only reason this function ever
        // runs - see its one real call site in sync/mod.rs) means the server
        // has invalidated every previously-cached UID for this mailbox, so
        // cached envelopes must be discarded - that's the actual RFC 7162
        // requirement ("empty the local cache of that mailbox"). But the
        // mailbox *row*'s own metadata (path/imap_path/special_use/
        // is_subscribed/...) is written by the unrelated
        // `update_mailbox_metadata` and has nothing to do with UIDVALIDITY -
        // deleting the whole row (the previous approach) silently wiped that
        // out too on every real UIDVALIDITY-triggered rebuild, not just on
        // first insert. Deleting only from `envelopes` (not `mailbox`)
        // satisfies the real requirement without the collateral damage.
        tx.execute(
        ...
        .chain_err_summary(|| {
            format!(
                "Could not clear cached envelopes of mailbox {} account {}",
        ...
        tx.execute(
            // ON CONFLICT only touches the sync-state columns this function
            // actually owns (uidvalidity/flags/highestmodseq) - metadata
            // columns are left alone if the row already exists, same
            // preserve-don't-clobber pattern `update_mailbox_metadata`
            // already uses for its own columns. A genuinely new row (this
            // is the very first thing to ever touch this mailbox_hash)
            // still needs safe defaults for the columns this function
            // doesn't know about, matching what `update_mailbox_metadata`
            // already provides on its own first insert.
            "INSERT INTO mailbox \
             (mailbox_hash, uidvalidity, flags, highestmodseq, path, imap_path, separator, \
              no_select, special_use, is_subscribed) \
             VALUES (?4, ?1, ?2, ?3, '', '', 0, 0, '', 0) \
    ...
    fn update_mailbox_metadata(
        ...
        let tx = self
            ...
            .transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)?;
        tx.execute(
            // `uidvalidity` is in the INSERT column list *and* the
            // ON CONFLICT SET, unlike the other new columns above - a
            // brand new row (this connection is the first to ever see
            // this mailbox) needs *some* non-NULL value there, since
            // `mailbox_state()` reads it back as a bare, non-Option type
            // (unlike max_uid/highestmodseq, which already tolerate
            // NULL). 0 is safe as a placeholder specifically because real
            // IMAP servers never issue UIDVALIDITY=0 (RFC 3501), so it
            // can never accidentally match a real server value and be
            // mistaken for genuine synced state - it just forces a real
            // resync the first time this mailbox is actually opened,
            // same as if the row didn't exist at all.
            // `COALESCE(mailbox.uidvalidity, 0)` in the SET clause is
            // what makes this self-healing rather than just forward-only:
            // caught live that an *existing* NULL row (written by this
            // same function before this fix existed) stayed NULL forever
            // otherwise, since a plain `ON CONFLICT` with no `uidvalidity`
            // in the SET list never touches it - `mailbox.uidvalidity`
            // (the current row's own value, not `excluded.uidvalidity`)
            // is kept as-is if it's already a real synced value, and only
            // healed to 0 if it's still NULL from before.
