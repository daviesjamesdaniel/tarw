    ...
    pub async fn examine_mailbox(
        ...
        if !force {
            match self.stream.as_ref()?.current_mailbox.is(&mailbox_hash) {
                ...
                // permission change.
                MailboxSelection::Examine { latest_response, .. }
                | MailboxSelection::Select { latest_response, .. } => {
                    return Ok(latest_response.clone());
                }
...
impl ImapConnection {
    ...
    pub async fn is_online(&mut self) -> Result<()> {
        ...
        match timeout(timeout_dur, self.connect()).await {
            Ok(Ok(())) => Ok(()),
            Err(err) | Ok(Err(err)) => {
                // Fix: the retry below used to call self.connect().await
                // directly, with no timeout wrapper at all - unlike the
                // first attempt above. That meant a slow/stuck first
                // attempt could be followed by a second attempt with no
                // upper bound of its own, making the worst-case wait
                // unpredictable instead of capped. Wrapping the retry in
                // the same timeout() means the worst case is now bounded
                // (roughly 2x timeout_dur) and reported as a normal timeout
                // error instead of an open-ended wait.
                self.stream = Err(err.clone());
