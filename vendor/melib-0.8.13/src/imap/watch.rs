...
pub async fn examine_updates(
    ...
    if let Some(new_envelopes) = conn.resync(mailbox_hash).await? {
        ...
        {
            // A push during IDLE can be a flags-only update (e.g. another
            // client marks a message Seen/Deleted) with no UID/ENVELOPE
            // data attached - skip those here instead of unwrapping, same
            // guard the second loop below already applies. Previously this
            // unconditional unwrap() panicked on exactly that server
            // response, and since it runs deep inside a tokio task on the
            // IDLE connection, the panic couldn't unwind and aborted the
            // whole process - reproduced live as background-while-minimised
            // crashes.
