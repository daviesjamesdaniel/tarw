...
impl From<io::ErrorKind> for ErrorKind {
    fn from(kind: io::ErrorKind) -> Self {
        match kind {
            ...
            // callers checking `is_network()`/`is_disconnected()` (the
            // app's own reconnect-and-retry) never saw it as one.
...
impl From<io::Error> for Error {
    fn from(err: io::Error) -> Self {
        ...
        let kind = if s.contains("failed to lookup address information") {
            ...
            // A connection-death `io::Error` (peer reset, refused, aborted,
            // timed out, ...) still carries a raw OS errno on Linux, so it
            // was landing here as a generic `OSError` instead of the
            // `Network`/`TimedOut` classification the `io::ErrorKind` match
            // above already knows how to make - meaning callers that check
            // `is_network()`/`is_timeout()`/`is_disconnected()` (e.g. this
            // app's own reconnect-and-retry) never saw it as a dead
            // connection at all. Deferring to that same classification for
            // exactly the io::ErrorKind variants it already handles, before
            // falling back to a raw errno for everything else.
            match err.kind() {
                io::ErrorKind::ConnectionRefused
                | io::ErrorKind::ConnectionReset
                | io::ErrorKind::ConnectionAborted
                | io::ErrorKind::NotConnected
                | io::ErrorKind::TimedOut => err.kind().into(),
                _ => ErrorKind::OSError(Errno::from_raw(errno)),
            }
        } else {
            err.kind().into()
        };
